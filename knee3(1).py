...
def recheck_endpoint(boundary, end_point):
    ...
    if pt_in_array(end_point, bound):
        ...
        min_idx = np.argmin([(end_point[0]-pt[0])**2+(end_point[1]-pt[1])**2 for pt in bound])
        return bound[min_idx]
...
def split_boundary_cv(bound, endpts):
    segments = []
...
def get_delta_xy(pt, reso, delta_dist, poly_fn=None, k=None):
    '''
...
def get_pt_direction(i, coords, poly_order=2):
    neigbors = [-3, -2, -1, 0, 1, 2, 3]
    neigbors = list(filter(lambda x : 0<x<coords.shape[1], np.add(neigbors, i)))
    # Curve fit
    z = np.polyfit([coords[0][n] for n in neigbors], 
                   [coords[1][n] for n in neigbors], 
                   poly_order)
    p = np.poly1d(z)
    k = p.deriv(1)(coords[0][i])
    if abs(k) > 1000:
        return ''
    else:
        return -1/k
...
def get_padding_region(x, y, direction, ref_boundary, reso, delta_dist, k, intersect_flag=True):
    new_line = []
    new_direction = None
    delta_x_px, delta_y_px = get_delta_xy((x,y), reso, delta_dist, k=k)
    new_pt_pos = (x+delta_x_px, y+delta_y_px)
    new_pt_neg = (x-delta_x_px, y-delta_y_px)
    if is_acute_angle((x,y), new_pt_pos, direction):
        inter_pts = get_line_segment((x,y), new_pt_pos)
        new_direction = (delta_x_px, delta_y_px)
        if intersect_flag and np.any([ pt_in_array(pt, ref_boundary) for pt in inter_pts]):
            inter_pts =  get_line_segment((x,y), new_pt_neg)
            new_direction = (-delta_x_px, -delta_y_px)
        new_line = new_line + inter_pts
        return new_line, new_direction
...
def padd(roi_fname):
# roi_fname = r"C:\Users\cwang\Desktop\2\HU DONG QI-20140521\roi.nii.gz"
    ...
    for slice_idx in z_nonzero_slices(roi):
        ...
        for target_label in candidate_label:
            ...
            for binary_slice in binary_slices:
                ...
                new_endpts = []
                if len(endpts) > 2:
                    new_endpts.append(endpts[0])
                    max_id = np.argmax([ (pt[0]-endpts[0][0])**2+(pt[1]-endpts[0][1])**2 for pt in endpts[1:] ])
                    new_endpts.append(endpts[max_id+1])
                else:
                    new_endpts = endpts
                ...
                try:
                    ...
                    # split the boundary
                    b_idx = np.argmax([ len(b) for b in boundary]) if len(boundary) > 1 else 0
                    new_endpts = [ recheck_endpoint(boundary[b_idx], pt) for pt in new_endpts ]
                    print('endpoints:', new_endpts)
                    boundaries = split_boundary_cv(boundary[b_idx], new_endpts)
                except:
                    pass
                    # plt.imshow(binary_slice)
                    # plt.show()
                    # plot_lines(boundary[0].squeeze(), endpts, color='blue')
                    # os.sys.exit()
                ...
                center_idx = len(coords[0])//2
                center_pt = (coords[0][center_idx], coords[1][center_idx])
                center_delta = get_delta_xy(center_pt, reso, 10, k=get_pt_direction(center_idx, coords))
                center_pt_pos = (center_pt[0]+center_delta[0], center_pt[1]+center_delta[1])
                ...
                try:
                    ...
                    for idx, (x, y) in enumerate(zip(coords[0,center_idx:-2], coords[1,center_idx:-2])):
                        norm_direct = get_pt_direction(idx+center_idx, coords)
                        padding_lines, direct = get_padding_region(x, y, prev_direction, ref_boundary, reso, delta_dist, norm_direct)
                        new_line += padding_lines
                        if direct is not None:
                            prev_direction = direct
                    prev_direction = direction
                    for idx, (x, y) in enumerate(zip(coords[0,2:center_idx][::-1], coords[1,2:center_idx][::-1])):
                        norm_direct = get_pt_direction(center_idx-idx-2, coords)
                        padding_lines, direct = get_padding_region(x, y, prev_direction, ref_boundary, reso, delta_dist, norm_direct)
                        new_line += padding_lines
                        if direct is not None:
                            prev_direction = direct
                ...
                tmp_slice = binary_closing(tmp_slice, generate_binary_structure(2,2), iterations=3).astype(np.int)
...
for case in os.listdir(all_data_path):
    roi_path = os.path.join(all_data_path, case, "roi.nii")
    # from Tool.DataProcress import get_array_from_path
    # roi = get_array_from_path(roi_path)
    padd(roi_path)
    # try:
