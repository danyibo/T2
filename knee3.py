...
def padd(roi_fname=None):
    # roi_fname = r"E:\Data\doctor_xie\knee_data\CAI QIN-20190323\roi.nii.gz"
    ...
    for slice_idx in z_nonzero_slices(roi):
        ...
        for target_label in candidate_label:
            ...
            for binary_slice in binary_slices:
                ...
                tmp_slice = binary_closing(tmp_slice, generate_binary_structure(2,2), iterations=2).astype(np.int)
                output_roi_slice[tmp_slice > 0] = target_label
        output_roi[...,slice_idx] = output_roi_slice

    print(np.sum(output_roi))
    nib.save(nib.Nifti1Image(output_roi, nii.affine, nii.header),
             os.path.join(roi_fname.replace('.nii', '_new.nii')))


all_data_path =r"Y:\DYB\2020832DATA\doctor_xie\roi"
# for case in os.listdir(all_data_path):
roi_path = os.path.join(all_data_path, "case22", "new_roi.nii")
padd(roi_path)
    # try:
